...
impl<S: BlockStore + Send + Sync> MetricsSources<S> {
    ...
    // The stall dump: ONE structured line, logged when /health first reports 503 in an
    // episode, answering "when the node stalls, what was it last doing?" — every field is a witness
    // we previously reconstructed by hand (last-span timestamps, manual inspection of the WAL).
    // Ages are seconds; -1 = never/idle this process.
