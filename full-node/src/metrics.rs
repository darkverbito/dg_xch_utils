...
mod tests {
    ...
    // ACTIVELY in flight (a window-batched coin_record INSERT freezes both progress witnesses on a
    // high-latency store). An in-flight confirm IS progress, so the verdict must be 200: killing
