    ...
    S: BlockStore + CoinStore + Send + Sync + 'static,
{
    ...
    // "not synced" (fail-closed, the stale-chain verdict).
...
mod ub_relay_gate_tests {
    ...
    // never queued for the NewUnfinishedBlock2 announce. Without the gate the driver would
    // validate the header only, cache the poisoned block, and queue the relay; every honest peer
    // that fetched it would run the generator, hit the error, and ban us for 600s. The
    // `ub_body_fail` counter doubles as the vacuity guard: it proves the
