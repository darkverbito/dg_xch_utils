    ...
    S: CoinStore + BlockStore + Send + Sync + 'static,
{
    ...
    pub async fn get_block_spends(
        ...
        // block's cost, so RPC load must not be able to stall the event loop.
...
pub fn build_rpc_tls_context() -> Result<RpcTlsContext, IoError> {
    ...
    // TLS 1.3 floor: server-side sockets refuse TLS 1.2. The RPC listener uses the same
    // server-side context rules as the p2p listener.
...
/// `get_fee_estimate` response. Field names and JSON types match the published RPC shape exactly,
/// so existing wallets and tooling parse it unchanged.
#[derive(Serialize)]
