...
| 51 | Malicious-generator DoS: `execute_block_generator_result` `export`ed the WHOLE puzzle output before cost-bounding it, so an adversarial block generator that emits a huge integer (`concat`/`substr` ladder, ~268 MB/arg) many times deep-copied the shared atom once per condition and exhausted memory (one 280,000-arg vector OOM-killed a 24 GiB builder), and one emitting a 600,000-deep CREATE_COIN list built an owned `SExp` whose `Drop` recursed and overflowed the native stack. chia never materializes the output: it walks the run allocator's `NodePtr`s, classifies each integer argument with `sanitize_uint` (bounded reads, no copy), charges condition cost INCREMENTALLY and bails at `MAX_BLOCK_COST_CLVM` / the first duplicate (test_mempool.py:2867-2869). Sub-point (also fixed): the ladder probes condition-arg byte-length sanitization — a leading-zero-padded oversized height atom, which chia's `sanitize_uint` rejects (chia expects `ASSERT_HEIGHT_ABSOLUTE_FAILED`; we previously stripped the pad and satisfied the lock). | harvest of `test_mempool.py::TestMaliciousGenerators` (Q2 correctness campaign #195) | FIXED — streaming, cost-bounded, arena-native condition parse: `ClvmRuntime::run_in_arena` (`core/src/clvm/runtime.rs`) leaves the result in the flat arena; `parse_and_apply_spend_from_arena` + `arena_condition` + `sanitize_uint_from_arena` (`core/src/consensus/block_generator.rs`) walk it iteratively, classify integer args without copying, charge cost incrementally + bail, and fail at the first duplicate CREATE_COIN — mirroring chia_rs `conditions.rs::parse_conditions` / `sanitize_int.rs::sanitize_uint` (chia-consensus 0.42.1). Regression tests: `core/tests/malicious_generators.rs` (all 10 vectors active, previously 8 `#[ignore]`d) + the historical-mainnet-block generator fixtures in `core/tests/block_generator.rs` (accept-path guard). | FIXED |
...
## Triage dispositions (investigated, NOT divergences)

Symptoms chased to a root cause that turned out **not** to be a code divergence. Recorded so the
same scare is not re-opened.

| Symptom | Disposition | Evidence |
|---|---|---|
| **#51 postgres genesis-sync "wedge" at height 45,696** — a `--genesis-sync` Postgres node built off PR #51 (`tip-follow-fixes` @ `8aa6af7`) reached ~45,695 then stopped advancing: `readahead_inflight:59 / resident_windows:0`, `generation` climbing (repeated stall-reclaim rebases), and a flood of peer `Close(Normal)` / `ResetWithoutClosingHandshake`. Initially suspected as a Q3–Q5 network-hardening regression (unknown-message disconnect / RATE_LIMITS_V3 / TLS-1.3 floor). | **NOT REPRODUCIBLE on `8aa6af7` — NOT a Q3–Q5 code regression.** Both backends sync clean past 45,696 on `8aa6af7`: throwaway **SQLite** genesis node → peak 83,519; throwaway **Postgres** genesis node → 45,407 → 46,495 → 47,519. The `Close(Normal)` flood is incidental noise (introducer hang-ups + public-listener churn — documented benign in `core/src/protocols/mod.rs`), and the stall-reclaim watchdog only fires with `peers_live == true` (`full-node/src/daemon.rs`), i.e. a **confirm-frontier stall**, not a fetch/peer failure. Root cause was **environmental to the original deploy** — most likely CNPG store-I/O contention producing the confirm-stall (same shape as the epyc-on-SAN `DataFileRead` starvation), which backpressures the pipeline into the `inflight:59 / resident:0` symptom. Structural reason it looked new: **no Postgres node had ever genesis-synced 0→45,696 before** — the fleet legs (`pg-0`/`epyc`) were `pg_basebackup`-seeded ~2M blocks, so `8aa6af7` was the first Postgres genesis-from-0, and it works. Static review found every Q3 peer-path change inert-on-outbound or byte-identical to the known-good base (`d46cc3c`), and the Q4 `BlockRecord` layout change (`record_compat`) has byte-parity round-trip tests. Advisory for the original deploy: check store I/O headroom (storage class / IOPS), CNPG CPU/memory limits, and the peer set. |
