...
//! byte-identical. The fallback decoder is proven against that pinned form, not against its own
//! inverse.
...
/// Downgrade every stored record blob to the legacy layout in place, reproducing the on-disk
...
async fn legacy_layout_store_opens_and_follows() {
    ...
    // Follow: new records land in the current layout on top of the legacy blobs, and the store
