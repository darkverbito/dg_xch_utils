...
/// A request whose list claims far more items than the handler will ever use would otherwise burn
/// seconds of parse CPU before the handler truncated it, so the truncation happens DURING decode:
...
///     parsed;
///   - a variable-size element type still parses the tail element-by-element, since the O(1) skip
///     is only sound for fixed-size elements.
///
/// A seek past the buffer end is TOLERATED: the outer full-consumption check then reads an empty
...
/// end, which keeps that behavior without a position/len underflow in
/// [`ChiaSerialize::from_bytes_exact`]'s consumed check; trailing garbage after a skip that lands
/// INSIDE the buffer is still rejected there.
///
/// No pre-allocation from the untrusted count: this decoder, like `Vec<T>::from_bytes`, starts
/// empty and grows per parsed element.
