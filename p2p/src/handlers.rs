...
pub trait FullNodeApi: Send + Sync {
    ...
    // The per-peer combined subscription cap. The store-blind default is the UNTRUSTED config
    // default; the daemon overrides it with the trusted-tier policy (200,000 untrusted /
    // 2,000,000 trusted). The cap is applied DURING decode of the two register arms, so it must be
    // resolvable before the message body is parsed.
    fn max_subscriptions(&self, _peer: &Bytes32, _host: Option<IpAddr>) -> u32 {
        200_000
    }
    // The decode-time list cap for `RequestCoinState.coin_ids`: 100,000 untrusted, 500,000
    // trusted.
    ...
    // The store-blind default ships no exempt networks, so localhost only; the daemon widens it
    // with the trusted-CIDR list. An unresolved host cannot be localhost — refuse.
...
/// new_signage_point_vdf, new_end_of_sub_slot_vdf and respond_compact_proof_of_time are sent only
/// by a TIMELORD. A mismatch is an invalid protocol message — close with the short (10 s) protocol
/// ban. The check runs BEFORE the body is decoded.
...
/// The decode-time list cap on `request_puzzle_state.puzzle_hashes`: the SQLite bound-variable
/// ceiling less headroom for the query's own parameters. Kept numerically in sync with
...
/// Close a misbehaving peer's connection, in three parts:
/// (1) enter the peer's REMOTE host into the timed ban list for `cause`'s duration, so a
    ...
    async fn dispatch(
        ...
        match msg.msg_type {
            ProtocolMessageTypes::Handshake => {
                ...
                // RATE_LIMITS_V3 responder mirror: if the peer advertises v3, advertise it back,
                // so the responder appends the capability to its reply and both sides exchange
                // ConfigureWindowSizes. Outbound dials keep the default set, which carries no v3,
                // so only inbound-initiated links ever activate it.
                ...
                // refusal happens right after the handshake completes and BEFORE any greeting, and
                // closes WITHOUT banning.
                ...
                // Greet the new peer by type the moment its handshake completes. With no peak
                // (empty store / blind api) nothing is sent.
            ...
            ProtocolMessageTypes::RequestPuzzleState => {
                // puzzle_hashes past the store batch bound are skipped at decode time.
            ...
            ProtocolMessageTypes::RequestCoinState => {
                // The cap bounds the PARSE, not just the handler: a RequestCoinState claiming
                // millions of coin_ids would otherwise burn seconds of CPU before truncation.
                let host = peer_host(peers, peer_id).await;
            ...
            ProtocolMessageTypes::ConfigureWindowSizes => {
                ...
                // capability was negotiated (we mirrored it in our handshake reply); otherwise,
                // and on any validation failure (empty / oversized / bounding one of OUR unlimited
                // types), this is an invalid handshake — close with the short protocol ban.
            ...
            ProtocolMessageTypes::Error => {
                // The `error` protocol message (code 255): a peer at protocol >= 0.0.35 reports a
                // handler-side failure in place of a typed reject. Decode, log a warning, and
                // carry on — no ban, no disconnect. Tolerant parse only; we do not emit Error
                // frames.
...
fn served(msg_type: ProtocolMessageTypes) -> bool {
    matches!(
        ...
        ProtocolMessageTypes::Handshake
            ...
            | ProtocolMessageTypes::RequestFeeEstimates
            // The `error` protocol message (code 255): tolerated and logged, never banned, so a
            // conforming peer's error report stays off the unknown-type close path.
            | ProtocolMessageTypes::Error
