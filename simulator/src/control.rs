//! The plain-HTTP chain-control endpoint a wallet e2e harness drives.
//! `POST /farm_block { address, guarantee_tx_block }` farms a block whose reward pays `address`;
//! `POST /get_blockchain_state` reports the peak. Funding and chain advance run over this endpoint,
//! not over the mTLS peer protocol the wallet syncs on.
...
pub(crate) fn spawn(
    ...
    let run_c = run.clone();
    // The node's own RPC dispatch, so the plain-HTTP facade serves the full RPC surface (coin
    // records, push_tx, mempool, auto-farming) the same way the mTLS server does.
...
async fn handle(
    ...
    let (status, payload) = match path.as_str() {
        ...
        // A minimal peak height for the wallet harness; the full `get_blockchain_state` (with
        // header_hash and the rest) is served by the node RPC below.
        "/height" => match blockchain_state(&node).await {
            Ok(v) => (StatusCode::OK, v),
            Err(e) => (StatusCode::OK, json!({ "success": false, "error": e })),
        },
        // Everything else the node's RPC already answers: reuse its dispatch and its envelope.
...
async fn farm_block(
    ...
    // A count of blocks to farm, default 1. `guarantee_tx_block` is always honored: the simulator
    // only farms transaction blocks.
    let blocks = req
        .get("blocks")
