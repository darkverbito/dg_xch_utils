...
| ede354c58 | 2023-07-13 | introduce the `error` protocol message | UNCLEAR `[ind]` — we neither emit nor specifically parse `Error` frames; today an inbound Error logs as unmatched. Resolve: confirm CNI 2.7.1 sender expectations (it sends Error for some rejects) and add a tolerant parse arm. |
