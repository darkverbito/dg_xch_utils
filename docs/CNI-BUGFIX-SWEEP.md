...
| commit | date | what it fixed | disposition |
|---|---|---|---|
| a1b12d321 | 2026-05-01 | **RATE_LIMITS_V3: window-based rate limits capability + ConfigureWindowSizes** | COVERED `[ind]` — full adoption at chia 2.7.1's posture. Constants exact: the 36-entry v3 table (13 requests at window 2, 23 responses unlimited), MAX_CONFIGURE_RATE_LIMITS_ENTRIES=256, Capability 5/6/7, message code 111 (`core/src/protocols/rate_limits_v3.rs`, pinned by `core/tests/rate_limits_v3_table.rs`). Gating mirrors chia: v3 NOT in the default outgoing set (chia `_capabilities`), the RESPONDER auto-mirrors an initiator's v3 and both sides exchange ConfigureWindowSizes with chia's validation (unknown skipped; empty/oversized/bounding-our-unlimited ⇒ close). Enforcement: v3-tabled types bypass the v2 time limiter; bounded requests admit through per-type in-flight receive windows (close + RATE_LIMITER ban on overflow, localhost bypassed as chia does), handler-task-scoped via RAII guards; outbound requests honor the PEER's windows at the oneshot senders, slots freed on reply/timeout (`V3Link` on every link: `core/src/protocols/mod.rs`, `p2p/src/handlers.rs`, `clients/src/websocket/mod.rs`). Deliberate deltas: we tolerate configure arriving after other traffic (chia requires it immediately post-handshake), and our outbound windows apply to localhost peers too (chia bypasses; stricter on ourselves is safe). Tests: `p2p/tests/t052_rate_limits_v3.rs` (mirror+activate, invalid configure closes, unnegotiated configure closes, window trip disconnects+bans, localhost bypass). |
...
| 0046a3a4e | 2026-03-26 | inbound TIMELORD connections only from localhost/exempt networks | COVERED `[ind]` — both halves closed. Connection accept: the inbound Handshake arm refuses a TIMELORD from a non-eligible host right after the handshake, close-without-ban exactly like chia's `should_accept_inbound` (`p2p/src/handlers.rs`; `FullNodeApi::accept_inbound_timelord` defaults to localhost-only — chia's empty `exempt_peer_networks` default — and the daemon widens it with the trusted-CIDR list via `TrustPolicy::host_trusted`). Message senders: the four timelord-class inbound types (new_infusion_point_vdf / new_signage_point_vdf / new_end_of_sub_slot_vdf / respond_compact_proof_of_time) are gated on the sender's handshake type = TIMELORD before decode, mirroring chia's `allowed_senders` check (`protocol_message_type_to_node_type.py`) — a mismatch closes with the short protocol ban. Tests: `p2p/tests/t051_timelord_gating.rs`. The FULL sender-type map for every message type remains the `f1556b4dc` row's scope. |
...
| 9491c6ee3 + 04b9d010b | 2026-01-12 | deficit round robin across peers in TransactionQueue.pop | COVERED `[ind]` — direct port of the post-fix `TransactionQueue`: per-peer fpc-ordered lanes, per-peer CLVM-cost deficits, cursor round-robin with lowest-top-cost replenishment, `max_tx_clvm_cost = MAX_BLOCK_COST_CLVM / 2` fallback for cost-less entries (`full-node/src/tx_queue.rs`). High-priority (trusted) lane unchanged; per-peer + aggregate admission caps kept. Tests port chia's own DRR vector (`chia_deficit_round_robin_vector_orders_by_affordability`) plus the discriminating interleave case chia's vector misses (`drain_interleaves_peers_by_cost_deficit_round_robin`, red→green). Documented delta: our drain is batch-total, so the 100-pop lane cleanup collapses to a state-identical post-drain reset. |
...
3. **`a1b12d321` — RATE_LIMITS_V3 window-based limits (DoS-hardening parity).** CLOSED —
   capability, table, configure exchange, and both enforcement halves (see the §4 row).
4. **`0046a3a4e` — inbound timelord gating (DoS).** CLOSED — localhost/trusted-CIDR accept
   gate + timelord-sender-only message gating (see the §4 row).
5. **`9491c6ee3` — cross-peer deficit round robin in the tx queue (fairness/DoS).** CLOSED —
   chia's DRR ported into `tx_queue.rs` (see the §4 row).
