// The weight-proof fork point against the LOCAL chain: iterate the chains' sub-epoch summaries
// positionally, find the last agreement, and back the long-sync start off two sub-epochs below
// it (clamping the first three to genesis). The mid-chain long-sync must start AT/NEAR that
// boundary — never from zero, and never blindly from the local peak when the offline gap saw a
// deeper reorg. Summaries hash-chain (`prev_subepoch_summary_hash`), so the top-down positional
// hash match used here is equivalent to a bottom-up full-map walk.
...
// offline-gap shape): NO fork detected, and the conservative start is the height of the
...
async fn agreement_up_to_sub_epoch_k_starts_the_long_sync_near_k_not_zero() {
    ...
    let fork = wp_fork_point(store.as_ref(), &received, SUB_EPOCH_BLOCKS)
        .await
        .expect("fork point");
    // Last agreement = local summary #5 → two below = summary #3.
    assert_eq!(
        fork,
        WpForkPoint::NoForkDetected {
            conservative: ses_height(3)
        },
        "the long sync must start at the two-below sub-epoch boundary"
    );
    match fork {
...
async fn divergent_top_summary_reports_the_fork_below_the_peak() {
    ...
    let fork = wp_fork_point(store.as_ref(), &received, SUB_EPOCH_BLOCKS)
        .await
        .expect("fork point");
    // Last agreement = local summary #4 → two below = summary #2.
    assert_eq!(
        fork,
        WpForkPoint::Diverged {
            fork_point: ses_height(2)
        },
        "a mismatched summary is a detected divergence at the conservative back-off"
    );
}

// Two summaries can have different blocks and still be identical — an agreement index ≤ 2
// clamps the fork point to genesis.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn shallow_agreement_clamps_to_genesis() {
    ...
    assert_eq!(
        ...
        "an agreement index <= 2 returns 0"
    );
}
