...
//     --test reorg_while_shed -- --ignored --test-threads=1
...
mod postgres {
    ...
    async fn postgres_reorg_while_shed_rebuilds_the_reorg_tier_and_lands_beside_a_writer() {
        let url =
        ...
        // The "was at tip" posture, then the shed + reorg + writer core.
        store.build_indexes().await.expect("build indexes at tip");
        run_reorg_while_shed(Arc::new(store)).await;

        let rows: Vec<(String,)> =
            sqlx::query_as("SELECT indexname FROM pg_indexes WHERE tablename = 'coin_record'")
                .fetch_all(&pool)
                .await
                .expect("index catalog");
        let names: Vec<&str> = rows.iter().map(|(n,)| n.as_str()).collect();
        assert!(
            names.contains(&"coin_record_confirmed_index"),
            "the confirmed_index BRIN is present after the shed reorg, got {names:?}"
        );
        assert!(
            names.contains(&"coin_record_spent_index"),
            "the reorg rebuilt the spent_index btree on demand, got {names:?}"
