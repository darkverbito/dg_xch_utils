...
// validated clean before the mutation (header_validation.rs pins required_iters for 9_054_611), so a
...
// Light-path (proof-of-space only, no VDF) required_iters — seeds ancestor records so the target's
