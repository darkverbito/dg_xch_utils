// Long-reorg AT SCALE. A reorg ~1500 deep is legal because there is NO reorg-depth cap: fork
// choice is weight-only and the coin rollback is unfloored. The fork walk is store-backed, and
// these suites pin its contract at depth 100 and 1000, on every backend:
//
