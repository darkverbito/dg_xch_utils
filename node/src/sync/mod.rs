    ...
    P: ConsensusPrimitives + Sync,
{
    ...
    /// pre-split monolith for every caller that does not pipeline.
    ...
    /// error precedence, reported deltas) is byte-identical to the pre-split monolith.
    ///
    /// # Errors
...
/// the lower height. Byte-identical verdicts to the pre-split inline drains.
#[must_use]
