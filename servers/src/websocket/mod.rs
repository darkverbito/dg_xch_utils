...
impl WebsocketServer {
    ...
    pub fn init(
        ...
        Ok(Arc::new(
            // TLS 1.3 floor: every server-side socket must refuse a 1.2 handshake. rustls'
            // default builder would accept 1.2 + 1.3, so pin the version set to 1.3 only
            // (`servers/tests/tls13_floor.rs`).
