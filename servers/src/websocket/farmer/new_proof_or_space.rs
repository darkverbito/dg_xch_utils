...
impl<T: PoolClient + Sized + Sync + Send + 'static> NewProofOfSpaceHandle<T> {
                ...
                if let Some(sp_data) = sp.sp_source_data.as_ref() {
                    let (cc, rc) = if let Some(vdf) = sp_data.vdf_data.as_ref() {
                        ...
                        return Err(Error::new(
                            ...
                            "Source Signature Did not contain any data, Cannot Sign Proof",
                        ));
                    };
                    ...
                    return Err(Error::new(
                        ErrorKind::InvalidInput,
                        "Source Signature Data Request But was Null, Cannot Sign Proof",
                    ));
                }
            } else {
                None
    ...
    async fn handle_partial(
        ...
        let p2_singleton_puzzle_hash = new_pos
            .proof
            .pool_contract_puzzle_hash
            .as_ref()
            .expect("Checked in previous method");
        if self
            .pool_state
            .read()
            .await
        ...
        if let Some(peer) = peers.read().await.get(&peer_id).cloned() {
            ...
            for sk in self.farmer_private_keys.values() {
                ...
                if pk.to_bytes() == respond_sigs.farmer_pk.bytes() {
                    ...
                    let taproot_sig = sign_prepend(&taproot_sk, &to_sign, &agg_pk);
                        ...
                        .await
                    {
                        Ok(resp) => {
                            if let Some(v) = self
                                ...
                                .get_mut(p2_singleton_puzzle_hash)
                            {
                                v.points_acknowledged_since_start += resp.new_difficulty;
                                v.current_points += resp.new_difficulty;
                                v.points_acknowledged_24h.push((Instant::now(), pool_dif));
                            }
                            #[cfg(feature = "metrics")]
                            ...
                            if pool_dif != resp.new_difficulty {
                                info!("New Pool Difficulty: {:?} ", resp.new_difficulty);
                                if let Some(v) = self
                                    .pool_state
                                    .write()
                                    .await
