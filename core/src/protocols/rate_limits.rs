...
fn v1_limit(t: ProtocolMessageTypes) -> Limit {
    ...
    match t {
        ...
        // The Unknown sentinel (a code the protocol does not define): a conservative
        // aggregate-limited default, so an unhandled code is never a limit bypass. The read loop
        // disconnects on it before the limiter matters.
...
mod tests {
    ...
    // The five late protocol rows (error / solve / solution_response / partial_proofs /
    // configure_window_sizes) pin their exact protocol numbers, never a default.
    #[test]
    ...
    // Incoming counters advance unconditionally: once over budget, they stay over — the counter
    // commits even when the message is a violation.
    #[test]
