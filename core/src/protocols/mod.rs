...
pub enum ProtocolMessageTypes {
    ...
    //Solver protocol
    Solve = 109,
    //Harvester partial proofs
    PartialProofs = 110,
    //Rate-limits-v3 handshake follow-up
    ConfigureWindowSizes = 111,
    //The error protocol message — see shared::ErrorMessage
    Error = 255,
}
                                ...
                                Message::Binary(bin_data) => {
                                    ...
                                    match ChiaMessage::from_bytes(&mut cursor, protocol_version) {
                                        Ok(chia_msg) => {
                                            ...
                                            // A message type outside the protocol enum
                                            // disconnects the peer with the short
                                            // INTERNAL_PROTOCOL_ERROR ban and a PROTOCOL_ERROR
                                            // (1002) close, BEFORE the rate limiter or any
                                            // dispatch sees it. The recognized-code set is pinned
                                            // by core/tests/protocol_message_codes.rs, so this
                                            // cannot fire on a conforming peer. Links without a
                                            // ban registry / resolved host (outbound clients)
                                            // still close, they just cannot host-ban.
                                            ...
                                            // RATE_LIMITS_V3: once negotiated,
                                            ...
                                            // violation. Localhost and exempt peers bypass window
                                            // enforcement, but still bypass v2 for v3 types.
