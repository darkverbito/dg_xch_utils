...
pub fn op_cons<'a, D: Dialect>(
    ...
    Ok((
        CONS_COST,
        SExp::Pair(PairBuf::Borrowed((first, rest.split()?.0))),
    ))
}

pub fn op_first<'a, D: Dialect>(
    args: &'a SExp<'a>,
    _max_cost: u64,
    _dialect: &'_ D,
) -> Result<(u64, SExp<'a>), ClvmError> {
...
pub fn op_raise<'a, D: Dialect>(
    args: &'a SExp<'a>,
    _max_cost: u64,
    _dialect: &'_ D,
) -> Result<(u64, SExp<'a>), ClvmError> {
    match args {
        SExp::Atom(atom) => Err(ClvmError::Raise(format!("{atom:?}"))),
        SExp::Pair(pair) => {
            if pair.rest().nullp() {
