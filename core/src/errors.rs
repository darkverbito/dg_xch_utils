...
pub enum ClvmError {
    ...
    BadEncoding,
    /// A condition parsed successfully but is invalid, carrying the exact consensus error code
    /// the condition parser assigns (e.g. `RESERVE_FEE_CONDITION_FAILED`, `COIN_AMOUNT_NEGATIVE`).
    /// The code is carried through the `ClvmError` -> `ChiaError` mapping
    /// (`map_condition_error`) instead of being collapsed.
...
pub enum ChiaError {
    ...
    // The SF9 canonical-serialization rule for the transactions generator.
    ComplexGeneratorReceived = 148,
    // The SF9 6,000-spend block limit.
    TooManySpends = 149,
}
