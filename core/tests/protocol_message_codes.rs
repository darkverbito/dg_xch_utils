// Message-type completeness against the protocol's message-type table, including the `error`
// protocol message.
//
// Completeness is load-bearing because the read loop disconnects a peer that sends a message type
// outside the enum. That disconnect is only safe if the recognized-code set equals the protocol's:
// a defined code we mapped to `Unknown` would make us ban conforming peers over an inbound `error`
// frame or a `configure_window_sizes` handshake follow-up.
...
/// Every message-type code the protocol defines below 102 (all 99 members).
...
/// The tail above 101: mempool updates (104-107), solution_response (108),
...
// Every defined code must round-trip through the enum, never collapsing to the `Unknown`
// sentinel.
#[test]
...
// Undefined codes must still collapse to `Unknown` — the disconnect set must not shrink either.
// 0 and 2 are unassigned; 4 was retired in favour of 66; 112..255-exclusive are unassigned.
#[test]
...
// The `error` message body (int16 code, str message, optional bytes data) decodes from the exact
// wire shape and round-trips byte-identically.
#[test]
fn error_message_wire_shape_matches_chia() {
    let v = ChiaProtocolVersion::default();
    // Hand-built encoding: int16 BE -13, "no fee" (u32 len + utf8), None tag.
    ...
    // The Some(data) arm: bytes stream as a u32 length + raw bytes, which is Vec<u8>'s wire shape.
